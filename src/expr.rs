...
impl Op {
    pub fn new(s: &str) -> Result<(&str, Self), String> {
        utils::tag("+", s)
            .map(|s| (s, Op::Add))
            .or_else(|_| utils::tag("-", s).map(|s| (s, Op::Sub)))
            .or_else(|_| utils::tag("*", s).map(|s| (s, Op::Mul)))
            .or_else(|_| utils::tag("/", s).map(|s| (s, Op::Div)))
    }
}
...
impl Expr {
    pub fn new(s: &str) -> Result<(&str, Self), String> {
        Self::new_operation(s)
            ...
            .or_else(|_| BindingUsage::new(s).map(|(s, binding)| (s, Self::BindingUsage(binding))))
            .or_else(|_| Block::new(s).map(|(s, block)| (s, Self::Block(block))))
    }
    pub fn new_operation(s: &str) -> Result<(&str, Expr), String> {
    ...
    pub fn new_number(s: &str) -> Result<(&str, Expr), String> {
